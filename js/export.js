...
// classWorkerURL precisa ser um arquivo LOCAL (mesma origem do site) para
// evitar erro de CORS ao criar o Worker. coreURL/wasmURL podem continuar
// vindo de CDN (sao carregados via fetch/importScripts, nao tem essa restricao).
...
async function ensureFFmpeg(){
  ...
  ffmpegLoading=(async()=>{
    ...
    // ESTE arquivo precisa ser local (mesma origem) - e o unico que da erro
    // de CORS se vier de CDN, porque o navegador cria um Worker a partir dele.
    try{
      await ff.load({coreURL:core,wasmURL:wasm,classWorkerURL:"vendor/ffmpeg/814.ffmpeg.js"});
    }finally{
      URL.revokeObjectURL(core);URL.revokeObjectURL(wasm);
    }
